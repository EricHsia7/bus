  ...
  for (let i = 0; i < size; i++) {
    for (let j = 0; j < size; j++) {
      if (data[i][j] === true) {
        ...
        elements += generateRoundedRectSVG(j * scale, i * scale, 1 * scale, 1 * scale, { tl: cornerRadius[0] * outerRadius * scale, tr: cornerRadius[1] * outerRadius * scale, br: cornerRadius[2] * outerRadius * scale, bl: cornerRadius[3] * outerRadius * scale }, fill, false);
      } else {
        // blank
        const cornerRadius = new Uint8Array(4);
        for (let k = 0; k < 4; k++) {
        ...
        if (cornerRadius[0] !== 0 || cornerRadius[1] !== 0 || cornerRadius[2] !== 0 || cornerRadius[3] !== 0) {
          elements += generateRoundedRectSVG(j * scale, i * scale, 1 * scale, 1 * scale, { tl: cornerRadius[0] * innerRadius * scale, tr: cornerRadius[1] * innerRadius * scale, br: cornerRadius[2] * innerRadius * scale, bl: cornerRadius[3] * innerRadius * scale }, fill, true);
        }
      }
    }
  }
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${size * scale} ${size * scale}">${elements}</svg>`;
}
