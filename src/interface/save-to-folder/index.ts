...
import { listFoldersWithContent } from '../../data/folder/index';
...
function generateElementOfItem(item: object): GeneratedElement {
  ...
  element.innerHTML = `<div class="css_save_to_folder_item_icon">${icons[item.icon]}</div><div class="css_save_to_folder_item_name">${item.name}</div>`;
  return {
    element: element,
    id: identifier
  };
