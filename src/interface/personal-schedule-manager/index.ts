...
function generateElementOfItem(item: PersonalSchedule): GeneratedElement {
  ...
  element.innerHTML = `<div class="css_personal_schedule_manager_item_name">${item.name}</div><div class="css_personal_schedule_manager_item_status"></div><div class="css_personal_schedule_manager_item_arrow">${getIconHTML('arrow_forward_ios')}</div>`;
  return {
    element: element,
    id: identifier
  };
