...
export function promptMessage(message: string, icon: string, duration: number = 1200): void {
  ...
  const prompt_css = `.css_prompt_animation_${promptID} {animation-name: css_prompt_${promptID};animation-duration: ${duration + baseDuration * 2}ms;animation-fill-mode: forwards;animation-timing-function: cubic-bezier(.21,.75,.1,.96);} @keyframes css_prompt_${promptID} { 0% {opacity: 0;transform: translateX(-50%) translateY(clamp(-45px, calc(${translateY}px - var(--b-cssvar-safe-area-bottom)), -20px)) scale(0.8);} ${Math.floor((baseDuration / (duration + baseDuration + 150)) * 100)}% {opacity: 1;transform: translateX(-50%) translateY(clamp(-45px,calc(${translateY}px - var(--b-cssvar-safe-area-bottom)),-20px)) scale(1);} ${Math.floor(((baseDuration + duration) / (duration + baseDuration + 150)) * 100)}% {opacity: 1;transform: translateX(-50%) translateY(clamp(-45px, calc(${translateY}px - var(--b-cssvar-safe-area-bottom)), -20px)) scale(1);} 100%{opacity: 0;transform: translateX(-50%) translateY(clamp(-45px, calc(${translateY}px - var(--b-cssvar-safe-area-bottom)), -20px)) scale(1);}} .css_prompt_animation_${promptID} .css_prompt_icon span.css_material_symbols_rounded {animation-name: css_prompt_icon_${promptID};animation-duration: ${baseDuration * 1.5}ms;animation-fill-mode: forwards;animation-timing-function: ease-in-out;} @keyframes css_prompt_icon_${promptID} { 0% {--b-cssvar-font-variation-settings-fill: 0;} 100% {--b-cssvar-font-variation-settings-fill: 1;}}`;
