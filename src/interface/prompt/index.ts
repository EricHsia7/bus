...
export function prompt_message(message: string, duration: number): void {
  ...
  var prompt_css = `.prompt_animation_${prompt_id}{animation-timing-function:cubic-bezier(0.080, 0.635, 0.455, 0.985);animation-name:prompt${prompt_id};animation-duration:${duration + duration_base * 2}ms;animation-fill-mode:forwards;animation-timing-function:ease-in-out}@keyframes prompt${prompt_id}{0%{opacity:0;transform:translateX(-50%) translateY(${80}px) scale(1);}${Math.floor((duration_base / (duration + duration_base + 150)) * 100)}%{opacity:1;transform:translateX(-50%) translateY(calc(${translateY}px - var(--safe-area-bottom))) scale(1);}${Math.floor(((duration_base + duration) / (duration + duration_base + 150)) * 100)}%{opacity:1;transform:translateX(-50%) translateY(calc(${translateY}px - var(--safe-area-bottom))) scale(1);}100%{opacity:0;transform:translateX(-50%) translateY(80px) scale(1);}}`;
