...
export function prompt_message(message: string, duration: number = 1200): void {
  ...
  var prompt_css = `.prompt_animation_${prompt_id}{animation-timing-function:cubic-bezier(0.080, 0.635, 0.065, 0.950);animation-name:prompt${prompt_id};animation-duration:${duration + duration_base * 2}ms;animation-fill-mode:forwards;animation-timing-function:ease-in-out}@keyframes prompt${prompt_id}{0%{opacity:0;transform:translateX(-50%) translateY(${80}px) scale(1);}${Math.floor((duration_base / (duration + duration_base + 150)) * 100)}%{opacity:1;transform:translateX(-50%) translateY(clamp(-45px,calc(${translateY}px - var(--safe-area-bottom)),-20px)) scale(1);}${Math.floor(((duration_base + duration) / (duration + duration_base + 150)) * 100)}%{opacity:1;transform:translateX(-50%) translateY(clamp(-45px,calc(${translateY}px - var(--safe-area-bottom)),-25px)) scale(1);}100%{opacity:0;transform:translateX(-50%) translateY(80px) scale(1);}}`;
