...
export function prompt_message(message: string, duration: number = 1200): void {
  ...
  const prompt_css = `.css_prompt_animation_${prompt_id} {animation-timing-function:cubic-bezier(.21,.75,.1,.96);animation-name: css_prompt_${prompt_id};animation-duration: ${duration + duration_base * 2}ms;animation-fill-mode: forwards;animation-timing-function: ease-in-out} @keyframes css_prompt_${prompt_id} { 0% {opacity: 0;transform: translateX(-50%) translateY(clamp(-45px, calc(${translateY}px - var(--b-cssvar-safe-area-bottom)),-20px)) scale(0.8);} ${Math.floor((duration_base / (duration + duration_base + 150)) * 100)}% {opacity: 1;transform: translateX(-50%) translateY(clamp(-45px,calc(${translateY}px - var(--b-cssvar-safe-area-bottom)),-20px)) scale(1);} ${Math.floor(((duration_base + duration) / (duration + duration_base + 150)) * 100)}% {opacity: 1;transform: translateX(-50%) translateY(calc(clamp(-45px, calc(${translateY}px - var(--b-cssvar-safe-area-bottom)), -20px)) scale(1);} 100%{opacity: 0;transform: translateX(-50%) translateY(clamp(-45px, calc(${translateY}px - var(--b-cssvar-safe-area-bottom)), -20px)) scale(1);}}`;
