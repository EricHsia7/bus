...
export function updatePropertiesField(Field: HTMLElement, properties: [], skeletonScreen: boolean): void {
  function updateProperty(thisElement: HTMLElement, thisProperty: object, previousProperty: object): void {
    function updateIcon(thisElement: HTMLElement, thisProperty: object): void {
      elementQuerySelector(thisElement, '.route_details_property_icon').innerHTML = icons[thisProperty.icon];
    }
    function updateValue(thisElement: HTMLElement, thisProperty: object): void {
      elementQuerySelector(thisElement, '.route_details_property_value').innerText = thisProperty.value;
    }
  ...
  var currentPropertySeatQuantity = elementQuerySelectorAll(Field, `.route_details_group_body .route_details_property`).length;
  if (!(propertyQuantity === currentPropertySeatQuantity)) {
    ...
    if (capacity < 0) {
      for (var o = 0; o < Math.abs(capacity); o++) {
        ...
        elementQuerySelector(Field, '.route_details_group_body').appendChild(thisPropertyElement.element);
      }
    } else {
      for (var o = 0; o < Math.abs(capacity); o++) {
        var propertyIndex = currentPropertySeatQuantity - 1 - o;
        elementQuerySelectorAll(Field, `.route_details_group_body .route_details_property`)[propertyIndex].remove();
      }
    }
  }

  for (var i = 0; i < propertyQuantity; i++) {
    var thisPropertyElement = elementQuerySelectorAll(Field, `.route_details_group_body .route_details_property`)[i];
