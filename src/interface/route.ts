...
export function updateRouteField(Field: HTMLElement, formattedRoute: object, skeletonScreen: boolean) {
  ...
  for (var i = 0; i < groupQuantity; i++) {
    ...
    for (var j = 0; j < itemQuantity[groupKey]; j++) {
      ...
      thisElement.querySelector('.overlapping_routes').innerHTML = thisItem.overlappingRoutes === null ? '目前沒有路線可顯示' : thisItem.overlappingRoutes.map((route) => `<div class="overlapping_route"><div class="overlapping_route_name">${route.name}</div><div class="overlapping_route_endpoints">${route.RouteEndPoints.text}</div></div>`).join('');
    }
  }
}
