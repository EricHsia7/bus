...
export function updateRouteField(Field: HTMLElement, formattedRoute: object, skeletonScreen: boolean) {
  ...
  for (var i = 0; i < groupQuantity; i++) {
    ...
    for (var j = 0; j < itemQuantity[groupKey]; j++) {
      ...
      thisElement.querySelector('.overlapping_routes').innerHTML = thisItem.overlappingRoutes === null ? '<div class="overlapping_route_message">目前沒有路線可顯示</div>' : thisItem.overlappingRoutes.map((route) => `<div class="overlapping_route"><div class="overlapping_route_title><div class="overlapping_route_icon">${icons.route}</div><div class="overlapping_route_name">${route.name}</div></div><div class="overlapping_route_endpoints">${route.RouteEndPoints.text}</div><div class="overlapping_route_actions"><div class="overlapping_route_action_button">查看路線</div><div class="overlapping_route_action_button">收藏路線</div></div></div>`).join('');
    }
  }
}
