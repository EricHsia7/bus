...
export async function updateNotification(telegramBotToken: string, telegramChatID: number): Promise<NotificationResponseObjectUpdate | false> {
  const url = await getNotificationAPIURL('update', [telegramBotToken, telegramChatID]);
  if (url === false) {
    return false;
  } else {
