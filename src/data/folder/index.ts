...
export async function createFolder(name: string, icon: string): Promise<boolean | string> {
  ...
  if (!Folders.hasOwnProperty(`f_${identifier}`)) {
    const existingFolder = await lfGetItem(8, `f_${identifier}`);
    if (!existingFolder) {
      ...
      await lfSetItem(8, `f_${identifier}`, JSON.stringify(object));
      return identifier;
    } else {
      return false;
    }
...
export async function updateFolder(folder: Folder): Promise<boolean> {
  if (['saved_stop', 'saved_route'].indexOf(folder.id) < 0 && !folder.default) {
    ...
    const existingFolder: string = await lfGetItem(8, folderKey);
    if (existingFolder) {
      const requestID = generateIdentifier('r');
      ...
      if (materialSymbols.indexOf(folder.icon) < 0) {
        ...
        await lfSetItem(8, folderKey, JSON.stringify(folder));
        return true;
      }
    } else {
      return false;
